...
- **`chat/`**: the `/chat` SPA — a themed shell (shared `/webui` design + tabbed sessions) that **embeds llama.cpp's own web UI in an iframe** per tab, for quick tok/s testing (the real chat client lives elsewhere, e.g. LibreChat). The iframe src is `/chatui`, which live-proxies the index HTML from a running `llama-server` replica (`api.chatUi` → first of `router._sortedPorts()`). It is served at a **top-level** path deliberately: llama.cpp's UI uses relative asset paths (`./_app/*`) that resolve to `/_app/*` — already proxied by the catch-all — while its absolute API calls (`/v1/*`, `/models/load`, `/props`) fall through to the router and stay router-governed (so the UI's model dropdown and load/unload are handled by the router, not a bare llama-server). Needs ≥1 loaded instance; the shell shows a "load a model first" state until then. Chat history is intentionally disposable (llama.cpp keeps it client-side/per-origin).
