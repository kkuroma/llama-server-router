...
- **`api.py`**: FastAPI app. Custom endpoints (`/router/*`, `/models/load`, `/models/unload`, `/dash`, `/translate`, `/chat`, `/v1/models`), a **`/webui` StaticFiles mount** (shared UI assets — see below), plus a **catch-all `/{full_path}` proxy** that wraps every other request into an "envelope" dict and hands it to `router.add_request()`. The `/webui` mount is registered before the catch-all so its paths are served locally, not forwarded. `api.router`/`api.gpu_monitor`/etc. are module-level globals set by `main.py` at startup.
- **`monitor.py`**: `GPUMonitor` (NVML util/VRAM history) and `StatusTimeline` (router status changes). Polled once/sec from a background task in `main.py`. Both keep a 2-hour rolling window. **`GPUMonitor` samples every detected NVIDIA GPU**, keeping a per-GPU history (`.gpus` list, `.snapshot()` serializes it); `/router/gpu` returns `{gpus: [...]}` and the dashboard shows one GPU's util+VRAM at a time with a selector.
...
- **`chat/`**: the `/chat` SPA — a themed shell (shared `/webui` design + a tab per live instance) that **embeds llama.cpp's own web UI in an iframe**, one per running `llama-server` replica, for quick tok/s testing (the real chat client lives elsewhere, e.g. LibreChat). **Each iframe src points DIRECTLY at that replica's own `host:port`** (built client-side from `window.location.hostname` + the instance port; tabs are derived from `/router`'s `instances` map). This is deliberate and load-bearing: the embedded UI's polling/generation traffic (`/props`, `/v1/*`, slots) must **never** re-enter the router. An earlier version proxied the UI through the router (a `/chatui` endpoint feeding the catch-all), which made that continuous UI traffic count as `AsyncRWLock` *readers* — and since readers starve writers, the write-lock `unloadModel` needs never drained, leaving the router stuck `serving` and unable to unload. Direct-embedding keeps all UI traffic on the instance; only the load/unload chips (explicit `/models/load` · `/models/unload` POST routes, one-shot writers) touch the router. Needs ≥1 loaded instance; the shell shows a "load a model first" state until then. Chat history is intentionally disposable (llama.cpp keeps it client-side/per-origin). Caveat: the instance ports (`LLM-base-port`, default 30000+) must be reachable from the browser, not just the router port.
...
- **Concurrency control**: `AsyncRWLock` — generation requests are *readers* (concurrent), load/unload are *writers* (exclusive, wait for in-flight requests to drain). Readers intentionally starve writers to maximize cache hits.
