...
There is no linter or build step. The app is `src/main.py` run under `asyncio`.
...
`tests/` holds two GPU integration cases, run with `./run_tests.sh` (nix dev shell, `pytest -m gpu`): `test_request_order.py` asserts a queued request for a second model is not overtaken by a later request for the resident one, and `test_concurrent_request.py` asserts N users on a model with `parallel = P` really do get `min(N, P)` simultaneous generations. Both spawn their own router from `tests/configs.toml` (ports, llama-server path, two model dirs, case sizes) and take the GPU for the duration; `tests/core/` holds the harness and the fixtures.
...
- **`chat/`**: the `/chat` SPA — a themed shell (shared `/webui` design + a tab per live instance) that **embeds llama.cpp's own web UI in an iframe**, one per running `llama-server` replica, for quick tok/s testing (the real chat client lives elsewhere, e.g. LibreChat). Tabs are derived from `/router`'s `instances` map; each iframe loads **`/instance/{port}/`** — the `instance_proxy` route (`api.py`), a **same-origin pass-through proxy** that forwards straight to `http://127.0.0.1:{port}/…` and **bypasses `router.add_request`/the RWLock entirely** (it streams, so SSE tok/s tick live). Two constraints drive this shape, and both are load-bearing:
  ...
  - **Off the scheduler/lock.** The embedded UI polls continuously (`/props`, etc.). If that traffic re-enters the router's catch-all it becomes per-GPU `AsyncRWLock` *readers* on the instance's GPUs, and since readers starve writers, the exclusive lock `unload_model` needs on those GPUs never drains → that GPU stuck `serving`, can't unload. `instance_proxy` never calls `add_request`, so UI traffic takes no reader slots. Only the load/unload chips (explicit `/models/load` · `/models/unload` POST routes, one-shot writers) touch the router.
  - llama.cpp's UI uses **relative** asset paths (`./_app/*`, which resolve under the `/instance/{port}/` prefix) but **absolute** API paths (`/v1/*`, `/props`). To keep the latter pinned to the replica instead of escaping to the router root, `instance_proxy` injects a small **fetch/EventSource/XHR shim** (`_INSTANCE_SHIM`) into the proxied index HTML that rewrites same-origin absolute paths to `/instance/{port}/…`. The HTML is buffered for injection; all other responses stream through raw.
...
5. **A dropped connection ends the request.** `api.py` races the future against a disconnect watcher (`_resolve_or_drop`) and the streaming generator calls `router.cancel_request(future)` from its `finally`. That cancels the forwarding task tracked in `router.request_tasks`, which closes the upstream socket so llama.cpp frees the slot, or removes the entry from `self.requests` if it never dispatched. A closed connection cannot be told apart from an explicit cancel and SSE cannot resume, so both end the request. The forwarders release their per-GPU reader lock through `asyncio.shield`, because a cancel interrupting that release would leak a reader and readers starve writers, which would wedge the GPU in `serving` forever.
...
- **Eviction**: `lru` (default) or `fifo`. Residents with queued demand are evicted last. Anti-starvation: a head-of-queue request whose model isn't loaded **holds the queue** once it has waited `QUEUE_HEAD_GRACE` (default 0s, so arrival order is absolute), which stops the cache-hit scan from serving newer requests for the resident model ahead of it; the swap then happens as soon as the in-flight work drains. `QUEUE_FORCE_LOAD_TIMEOUT` (300s) remains the last resort that force-loads past a busy GPU.
- **Crash recovery**: `_reap_dead()` drops bookkeeping for exited processes; the model auto-reloads on its next request.
- **Idle metadata supervisor**: `/v1/models` should report real llama.cpp model info, but llama-server only exposes a model's rich `meta` (modalities, `n_ctx`, params) *while that model is loaded*, and when the router has **zero** replicas there is no llama.cpp process to ask at all. So the router keeps a bare `llama-server --models-preset --no-models-autoload` on a reserved port (`LLM-base-port - 1`, `CUDA_VISIBLE_DEVICES=""`) alive **only while nothing is loaded**: it holds no VRAM and no GPU, but still serves `/v1/models`/`/props` for the whole preset. `_reconcile_idle_supervisor` (called from `start`/`load_model`/`unload_model`, serialized by `_idle_lock`) enforces the invariant — spawn it when `_loaded_models()` is empty, kill it the instant a real replica exists (a live replica is a better proxy source). It is tracked in `self._idle_proc`, deliberately **out of `self.processes`**, so it never counts as a loaded replica. `models_report()` (behind `/v1/models`) proxies a live supervisor via `_proxy_port()` (a real replica first, else the idle one) for llama.cpp's native fields, then overlays our per-request `context_length` = `c / parallel` (llama.cpp reports neither `context_length` nor a per-slot `n_ctx`); it falls back to a synthesized `ids + context_length` listing if no supervisor is reachable. Per-model context comes from `model_context_windows()`, which parses the presets INI (`c`/`ctx-size` and `np`/`parallel`, `[*]` global fallback) once and caches it.
